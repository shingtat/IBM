    ...
    passport.use('local-signup', new LocalStrategy({
        ...
        function(req, username, password, done) {
           ...
           // Use Cloudant query to find the user just based on user name
            var db = Cloudant.use(dbname);
            db.find({selector:{username:username}}, function(err, res) {
                if (err){
                    ...
                    return done(null, false, { message : "This username already exists. Please choose a different username." } );
                }

                // create the new user
                var hash_pass = bcrypt.hashSync(password, 10);
                body.password = hash_pass;
                             //   var user = { username:username, password:hash_pass, gender: body.gender };
